<template>
  <div class="min-h-screen bg-gray-50">
    <div class="px-4 py-8 mx-auto max-w-6xl">
      ...
      <button
          @click="goBack"
          class="flex items-center mb-6 space-x-2 text-gray-700 transition-colors cursor-pointer hover:text-gray-900"
      >
      ...
      <div v-else>
        <!-- Main Card -->
        <div class="p-8 mb-6 bg-white rounded-3xl border-2 border-gray-200 md:p-10 animate-card-entrance">
          <div class="flex justify-between items-start mb-8">
            ...
            <div class="flex items-start space-x-6">
              ...
              <!-- Playlist Info -->
              <div>
                <h1 class="mb-3 text-3xl text-gray-700 font-regular">{{ playlist.name }}</h1>
          ...
          <div class="grid grid-cols-3 gap-8 mb-6">
            <div>
              ...
              <p class="text-xl font-medium text-gray-700">{{ formattedDuration }}</p>
            </div>
            <div>
              <p class="mb-1 text-sm text-gray-600">Genre</p>
              <p class="text-xl font-medium text-gray-700">{{ formattedGenres }}</p>
            </div>
            <div>
              <p class="mb-1 text-sm text-gray-600">Total Tracks</p>
              <p class="text-xl font-medium text-gray-700">{{ playlist.total_tracks }}</p>
            </div>
          </div>
          ...
          <button
              ...
              :class="playlist.mode === 'healing' ? 'bg-blue-500 hover:bg-blue-600 hover:shadow-lg' : 'bg-orange-500 hover:bg-orange-600 hover:shadow-lg'"
          >
        ...
        <div class="p-8 mb-6 bg-white rounded-3xl border-2 border-gray-200 md:p-10 animate-card-entrance">
          ...
          <div class="space-y-3">
            <div
                ...
                :key="index"
                class="flex justify-between items-center p-4 bg-gray-50 rounded-xl transition-colors hover:bg-gray-100"
            >
              <div class="flex items-center space-x-4">
                <div
                    class="flex justify-center items-center w-10 h-10 font-semibold text-gray-600 rounded-lg transition-colors duration-500"
                    :class="playlist.mode === 'healing' ? 'bg-blue-100' : 'bg-orange-100'"
                >
                  {{ index + 1 }}
                </div>
                <div>
                  <p class="font-semibold text-gray-900">{{ track.name }}</p>
              ...
              <div class="flex items-center space-x-2 text-gray-600">
                ...
                <span class="text-sm">{{ formatTrackDuration(track.duration) }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <button
          ...
          class="flex justify-center items-center px-6 py-4 space-x-2 w-full font-semibold text-red-600 bg-white rounded-2xl border-2 border-red-300 transition-all duration-300 cursor-pointer hover:bg-red-50 hover:shadow-md"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    >
      <div
          @click.stop
          class="p-8 w-full max-w-md bg-white rounded-2xl shadow-2xl animate-modal-entrance"
      >
        <!-- Icon -->
        ...
        <button
            @click="showDeleteModal = false"
            class="absolute top-4 right-4 text-gray-400 transition-colors cursor-pointer hover:text-gray-600"
        >
        ...
        <div class="mb-6 text-center">
          <h2 class="mb-3 text-2xl font-bold text-gray-900">Delete Session</h2>
        ...
        <div class="flex space-x-3">
          <button
              @click="showDeleteModal = false"
              class="flex-1 px-6 py-3 font-semibold text-gray-700 bg-white rounded-lg border-2 border-gray-300 transition-all duration-300 cursor-pointer hover:bg-gray-50 hover:shadow-md"
          >
            Cancel
          </button>
          <button
